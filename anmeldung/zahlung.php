 ...
 * Nur für zugelassene Anmeldungen; nach Zahlungseingang wird auf das Ticket verwiesen.
...
if ($rec === null) {
    x25_out(x25_page('Link ungültig', '<h1>Dieser Zahlungslink ist ungültig.</h1><p>Bitte nutzen Sie den Link aus Ihrer Zusage-Mail oder schreiben Sie an <a href="mailto:' . x25_e($C['mail_to']) . '">' . x25_e($C['mail_to']) . '</a>.</p>'), 404);
}
if ($rec['status'] !== 'zugelassen') {
    x25_out(x25_page('Keine Zahlung offen', '<h1>Für diese Anmeldung ist keine Zahlung offen.</h1><p>Stand: ' . x25_e(X25_STATUS[$rec['status']] ?? $rec['status']) . '. Bei Fragen schreiben Sie an <a href="mailto:' . x25_e($C['mail_to']) . '">' . x25_e($C['mail_to']) . '</a>.</p>'), 409);
}
...
if ($rec['payment_status'] === 'bezahlt') {
    x25_out(x25_page('Bezahlt', $intro . '<div class="card ok"><h2 style="margin-top:0">Vielen Dank, Ihre Zahlung ist eingegangen.</h2><p>Ihr Ticket ' . x25_e($rec['ticket_no'] ?? '') . ' wurde per E-Mail versandt.</p><a class="btn" href="' . x25_e(x25_ticket_url($rec)) . '">Ticket öffnen</a></div>'));
}
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && (string)($_POST['weg'] ?? '') === 'rechnung') {
    try {
        ...
        x25_out(x25_page('Fehler', $intro . '<div class="card warn"><p>Die Rechnung konnte gerade nicht erstellt oder versandt werden. Bitte versuchen Sie es in einigen Minuten erneut oder schreiben Sie an <a href="mailto:' . x25_e($C['mail_to']) . '">' . x25_e($C['mail_to']) . '</a>.</p></div>'), 500);
    }
...
if (($rec['payment_method'] ?? '') === 'rechnung' && !empty($rec['invoice_no'])) {
    $body .= '<div class="card ok"><h2 style="margin-top:0">Rechnung ' . x25_e($rec['invoice_no']) . ' ist unterwegs.</h2>'
        . '<p>Wir haben Ihnen die Rechnung per E-Mail gesandt (Zahlungsziel ' . x25_e(x25_date($rec['invoice_due'], 'd.m.Y')) . '). Nach Zahlungseingang erhalten Sie Ihr Ticket und alle weiteren Informationen.</p>'
        . '<a class="btn" href="' . x25_e(x25_invoice_url($rec)) . '">Rechnung ansehen / als PDF speichern</a></div>';
    if ($paypalOn) { $body .= '<p class="meta">Sie möchten doch lieber sofort per PayPal zahlen? Dann nutzen Sie die Schaltflächen unten; die Rechnung gilt dann als erledigt.</p>'; }
} else {
    $body .= '<p>Bitte wählen Sie Ihren Zahlungsweg. Mit dem Zahlungseingang ist Ihr Platz verbindlich; Sie erhalten dann Ihr Ticket und alle weiteren Informationen.</p>';
}
...
if ($paypalOn) {
    ...
    $body .= '<p class="meta">PayPal ist derzeit nicht verfügbar [TBD: PayPal-Zugang in config.php eintragen]. Bitte zahlen Sie per Rechnung.</p>';
}
$body .= '</div>';
if (($rec['payment_method'] ?? '') !== 'rechnung') {
    $body .= '<div class="card"><h2 style="margin-top:0">Rechnung</h2><p class="meta">Sie erhalten die Rechnung per E-Mail (Zahlungsziel ' . (int)$C['payment_days'] . ' Tage, Überweisung). Das Ticket kommt nach Zahlungseingang.</p>'
