...
Stand 17.08.2026. Dieses Verzeichnis ist der komplette Inhalt des GitHub-Repos, das Hostinger nach `public_html` ausrollt.
Domain: **https://25-experts.de/** (Domain und E-Mail liegen bei Hostinger). Kein n8n, kein Netlify, kein Resend:
Das Anmeldeformular wird von `anmeldung/send.php` auf dem Hostinger-Webspace verarbeitet und verschickt die Mails über ein Hostinger-Postfach.
...
.htaccess                     HTTPS-Zwang, www → 25-experts.de, saubere URLs (/format), Caching, Sicherheits-Header, 404
404.html, index.html, format.html, editionen.html, neutralitaetskodex.html, sponsoring.html, ueber-uns.html,
...
anmeldung/send.php            Formular-Handler (PHP 8.1+, PHPMailer per SMTP)
anmeldung/config.example.php  Vorlage für anmeldung/config.php (Zugangsdaten, Empfänger, Edition) – config.php liegt NICHT im Repo
anmeldung/.htaccess, anmeldung/lib/  Zugriffsschutz; PHPMailer (LICENSE liegt bei)
...
## Teil C: Formular scharf schalten (Postfach + config.php)
...
10. **config.php anlegen:** hPanel → **Dateien → Dateimanager** → `public_html/anmeldung/` → `config.example.php` öffnen → Inhalt kopieren → „Neue Datei" `config.php` → einfügen und ausfüllen:
    ...
    - `RATE_SALT` = irgendeine lange Zufallszeichenfolge
...
11. **Test-Anmeldung:** https://25-experts.de/editionen/change-management/#anmeldung ausfüllen und absenden → Weiterleitung auf die Danke-Seite → zwei Mails: Benachrichtigung an `MAIL_TO` (Antworten geht direkt an den Anmelder), Bestätigung an die eingetragene Adresse (Betreff „Ihre Anmeldung ist eingegangen · 25 CHANGE MANAGEMENT EXPERTS"). Auch einmal mit deaktiviertem JavaScript testen (Redirect statt JSON) und einmal ein Pflichtfeld leer lassen (Fehlermeldung im Formular).
...
- `send.php` prüft Pflichtfelder, E-Mail-Syntax, Ebene/Kategorie-Werte, Honeypot (`website`), Origin-Header, entfernt Zeilenumbrüche aus Kopfzeilenfeldern (kein Header-Injection), begrenzt je IP-Hash 5 Anmeldungen/Stunde (Sperrdateien ohne Klartext-IP in `sys_get_temp_dir()`), speichert nichts dauerhaft, protokolliert keine personenbezogenen Daten (nur „Versand fehlgeschlagen" mit SMTP-Fehlertext im PHP-Errorlog).
- `anmeldung/config.php`, `anmeldung/lib/`, `tools/`, `README*.md`, `.git*` sind per `.htaccess` gegen Direktaufruf gesperrt.
...
`php -S 127.0.0.1:8765 -t repo/` im Ordner `07-deploy/` und `anmeldung/config.php` mit `MAIL_TRANSPORT = 'file'` anlegen: Mails landen als `.eml` in `MAIL_DUMP_DIR` statt verschickt zu werden. Automatisierter Test: `python3 07-deploy/test/run_tests.py` (43 Prüfungen: JSON- und Formular-POST, Validierung, Honeypot, Injection, Rate-Limit, SMTP-Fehlerpfad).
